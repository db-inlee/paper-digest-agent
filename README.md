...
> 매일 쏟아지는 AI 논문, 팀원들과 함께 골라보세요.

**Paper Digest Agent**는 HuggingFace Daily Papers에서 LLM/Agent 관련 논문을 매일 자동으로 수집하고 분석해주는 도구입니다. 분석된 논문은 Slack 채널에 공유되고, 팀원들이 투표와 댓글로 함께 의사결정할 수 있습니다.

- AI가 논문의 실용성·구현 가능성·신뢰도를 평가하고 한글 리포트를 생성합니다
- Slack에서 바로 투표(실무 적용 / 아이디어 / 패스)하고 댓글로 의견을 나눌 수 있습니다
- 웹 대시보드에서 분석 결과를 상세히 확인하고, 팀의 투표 현황을 한눈에 볼 수 있습니다
- 관심 논문을 arXiv URL로 직접 추가하면 즉시 분석이 시작됩니다
...
팀원들도 접속하고 Slack 버튼이 동작하려면 서버를 외부에 공개해야 합니다.
[ngrok](https://ngrok.com/)을 사용하면 무료로 간단하게 할 수 있습니다.
...
Slack과 연동하면 논문 리포트가 채널에 자동 전송되고, 팀원들이 버튼으로 바로 투표·댓글할 수 있습니다.
...
`.env`에서 스케줄러를 켜면, 매일 아침 지정한 시각에 논문 수집·분석이 자동으로 실행되고, 완료되면 Slack 채널에 리포트가 자동 전송됩니다.

```bash
# .env
SCHEDULER_ENABLED=true       # 자동 실행 켜기
SCHEDULER_CRON_HOUR=9        # 매일 오전 9시
...
웹 대시보드에서는 분석 결과를 상세히 확인하고, 팀의 투표·댓글 현황을 한눈에 볼 수 있습니다.
